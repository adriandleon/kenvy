...
abstract class GenerateKenvyTask : DefaultTask() {
    ...
    init {
        ...
        generatedVisibility.convention(GeneratedVisibility.PUBLIC.value)
    ...
    fun generate() {
        ...
        val visibility = GeneratedVisibility.fromValue(generatedVisibility.get())

        if (visibility == GeneratedVisibility.PUBLIC) {
            logger.lifecycle(
                "Kenvy: generated declarations are public by default. " +
                "API-surface-sensitive shared modules should set " +
                "`generatedVisibility.set(\"internal\")` in the kenvy extension. " +
...
internal fun buildGeneratedSource(
    ...
    visibility: GeneratedVisibility = GeneratedVisibility.PUBLIC,
    ...
    return buildString {
        ...
        appendLine()
        appendLine("${visibility.prefix}${mode.objectKeyword} $name {")
        for (resolved in resolvedValues) {
...
internal enum class GeneratedVisibility(val value: String, val prefix: String) {
    PUBLIC("public", ""),
    INTERNAL("internal", "internal ");

    companion object {
        fun fromValue(value: String): GeneratedVisibility =
            values().firstOrNull { it.value == value }
                ?: throw GradleException(
                    "Kenvy: Unsupported generatedVisibility '$value'. " +
                    "Use '${PUBLIC.value}' or '${INTERNAL.value}'."
                )
    }
}
